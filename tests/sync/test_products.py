...
def test_new_multi_variant_product_creates_template_and_variants(session):
    ...
    assert all(e.shopify_fingerprint == e.erpnext_fingerprint for e in entities)
...
def test_update_title_and_description_updates_template_only(session):
    ...
    expected_fingerprint = fingerprint(canonicalize("product", updated))
    assert template_entity.shopify_fingerprint == expected_fingerprint
    assert template_entity.erpnext_fingerprint == expected_fingerprint
